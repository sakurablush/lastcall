...
**Requirements:** Node ≥ 18 runtime (`engines` in `package.json`). CI uses Node 22 for the test toolchain and a separate **Bun** job for runtime compatibility.
...
GitHub Actions (`.github/workflows/ci.yml`) additionally runs `build`, `docs:build`, `npm pack --dry-run`, and bundle size check (Node 22), plus a **Bun** job (`npm test` + `build`).
