...
Expands to: `lint` → `format:check` → `typecheck` → `test:coverage` → `audit`

When you changed `docs/` or `website/`, also run `npm run docs:build` before merge (CI workflow runs it after `npm run ci`).
...
```bash
npm run lint && npm run format:check && npm run typecheck && npm run test:coverage && npm run audit
```

If docs changed, add `npm run docs:build`.
