...
describe('signal handling', () => {
  ...
  it('listHandlers sorts handlers when phase is outside configured phases', () => {
    const lastcall = createLastcall({
      autoExit: false,
      signals: [],
      phases: ['cleanup'],
    ...
    lastcall.register('pre', vi.fn(), { phase: 'pre', priority: 50 });
    lastcall.register('cleanup', vi.fn(), { phase: 'cleanup', priority: 50 });

    expect(lastcall.listHandlers().map((handler) => handler.name)).toEqual(['cleanup', 'pre']);
  });
